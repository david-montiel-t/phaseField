                                            ...
                                            [[maybe_unused]] Vector<double>    &vector_IC)
{
  ...
  double centerx = 0.5*userInputs.domain_size[0];
  double centery = 0.5*userInputs.domain_size[1];
  double centerz = 0.5*userInputs.domain_size[2];
  ...
  for (unsigned int i = 0; i < dim; i++)
    {
      r += (p(i)-center[i]) * (p(i)-center[i]) / ellipsoid_denoms[i];
    }
  r = sqrt(r);
