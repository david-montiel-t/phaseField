...
public:
	customPDE(userInputParameters<dim> _userInputs): MatrixFreePDE<dim,degree>(_userInputs) , userInputs(_userInputs) {
        ...
        Kn1[2][2] = 18.0 * (interfacial_energy_33/2000.0)*(interfacial_energy_33/2000.0)/W;
...
private:
    ...
    double interfacial_energy_33 = userInputs.get_model_constant_double("interfacial_energy_33");
    ...
    dealii::Tensor<1,dim> initial_radius = userInputs.get_model_constant_rank_1_tensor("initial_radius");
    double c_matrix_initial = userInputs.get_model_constant_double("c_matrix_initial");
        ...
        for (; this->currentIncrement<=this->userInputs.totalIncrements; ++this->currentIncrement){
            ...
            if (this->userInputs.outputTimeStepList[this->currentOutput] == this->currentIncrement) {
                ...
                if (n_ouputs_so_far > 1){
                        this->pcout << list_of_integrated_postprocessed_fields[n_ouputs_so_far-2][0] << " " << list_of_integrated_postprocessed_fields[n_ouputs_so_far-1][0] << " " << std::abs(list_of_integrated_postprocessed_fields[n_ouputs_so_far-2][0] - list_of_integrated_postprocessed_fields[n_ouputs_so_far-1][0]) << std::endl;
                    if (std::abs(list_of_integrated_postprocessed_fields[n_ouputs_so_far-2][0] - list_of_integrated_postprocessed_fields[n_ouputs_so_far-1][0]) < equilbrium_tol){
