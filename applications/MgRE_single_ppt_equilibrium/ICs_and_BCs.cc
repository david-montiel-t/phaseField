...
void customPDE<dim,degree>::setInitialCondition(const dealii::Point<dim> &p, const unsigned int index, double & scalar_IC, dealii::Vector<double> & vector_IC){
    ...
    double x_denom = (initial_radius[0])*(initial_radius[0]);
    double y_denom = (initial_radius[1])*(initial_radius[1]);
    double z_denom = (initial_radius[2])*(initial_radius[2]);

    double initial_interface_coeff = 0.05;
    double initial_radius = 1.0;
    double c_matrix = c_matrix_initial;
