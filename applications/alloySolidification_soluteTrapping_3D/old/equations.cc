...
// let's rotate the normal to crystal frame
...
scalarvalueType theta;
for (unsigned i=0; i< phi.n_array_elements;i++){
    theta[i] = std::atan2(phix[1][i],phix[0][i]) - theta0*pi/180.0;
}
c4th = std::cos(constV(4.0)*theta); 
...
// --- Setting the expressions for the terms in the governing equations ---
  ...
  // The azimuthal angle
  //scalarvalueType theta = std::atan2(normal[1],normal[0]) - theta0*pi/180.0;
