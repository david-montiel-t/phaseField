...
#define h1V_5tho (n1*n1*n1*(10.0-15.0*n1+6.0*n1*n1))
#define hn1V_5tho (30.0*(n1-1.0)*(n1-1.0)*n1*n1)
#define h2V_5tho (n2*n2*n2*(10.0-15.0*n2+6.0*n2*n2))
#define hn2V_5tho (30.0*(n2-1.0)*(n2-1.0)*n2*n2)
#define h3V_5tho (n3*n3*n3*(10.0-15.0*n3+6.0*n3*n3))
#define hn3V_5tho (30.0*(n3-1.0)*(n3-1.0)*n3*n3)
...
for (unsigned int i=0; i<dim; i++){
for (unsigned int j=0; j<dim; j++){
	  E2[i][j]= constV(0.5)*(ux[i][j]+ux[j][i])-( sfts1[i][j]*h1V_5tho + sfts2[i][j]*h2V_5tho + sfts3[i][j]*h3V_5tho);
}
}
...
for (unsigned int i=0; i<dim; i++){
for (unsigned int j=0; j<dim; j++){
	E4[i][j] = constV(sfts_linear1[i][j])*h1V_5tho + constV(sfts_linear2[i][j])*h2V_5tho + constV(sfts_linear3[i][j])*h3V_5tho;

	nDependentMisfitAC1 -= S[i][j]*(cbn1V*E4[i][j] + sfts1[i][j]*hn1V_5tho);
	nDependentMisfitAC2 -= S[i][j]*(cbn2V*E4[i][j] + sfts2[i][j]*hn2V_5tho);
	nDependentMisfitAC3 -= S[i][j]*(cbn3V*E4[i][j] + sfts3[i][j]*hn3V_5tho);
}
}
